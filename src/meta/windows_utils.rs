...
pub fn get_file_data(path: &Path) -> Result<(Owner, Permissions), io::Error> {
    ...
    let owner = match unsafe { lookup_account_sid(owner_sid_ptr) } {
        ...
        Err(_) => {
            // Attempt to resolve foreign SID information
            match resolve_foreign_sid_permissions(unsafe { &*(owner_sid_ptr.0 as *const windows::Win32::Security::SID) }, path) {
                Ok(resolved_name) => {
                    // Use resolved name for foreign SID
                    resolved_name
                }
                Err(_) => {
                    // If resolution fails, use generic foreign SID indicator
                    "Foreign\\Unknown".to_string()
                }
            }
        }
    };
    ...
    let group = match unsafe { lookup_account_sid(group_sid_ptr) } {
        ...
        Err(_) => {
            // Attempt to resolve foreign SID information
            match resolve_foreign_sid_permissions(unsafe { &*(group_sid_ptr.0 as *const windows::Win32::Security::SID) }, path) {
                Ok(resolved_name) => {
                    // Use resolved name for foreign SID
                    resolved_name
                }
...
fn resolve_foreign_sid_permissions(
    sid: &windows::Win32::Security::SID,
    _file_path: &Path,
) -> io::Result<String> {
    use windows::Win32::Security::{LookupAccountSidW, SID_NAME_USE};
    use windows::Win32::Foundation::{ERROR_NONE_MAPPED, GetLastError};
    ...
    unsafe {
        ...
        if LookupAccountSidW(
            ...
            if error == ERROR_NONE_MAPPED {
                // Foreign SID that can't be resolved - show SID string instead
                Ok(format!("Foreign\\SID-{:?}", sid_use.0))
            } else {
...
unsafe fn lookup_account_sid(sid: PSID) -> Result<(Vec<u16>, Vec<u16>), std::io::Error> {
    ...
    loop {
        ...
        if result.is_ok() {
            ...
            // Attempt foreign SID resolution as fallback
            match resolve_foreign_sid_permissions(
                unsafe { &*(sid.0 as *const windows::Win32::Security::SID) },
                Path::new("") // dummy path, function parameter is unused
            ) {
                Ok(resolved) => {
                    // Parse "Domain\Username" format and convert to Vec<u16> buffers
                    if let Some(separator_pos) = resolved.find('\\') {
                        let domain_str = &resolved[..separator_pos];
                        let name_str = &resolved[separator_pos + 1..];
                        
                        // Convert to UTF-16 and add null terminators
                        let mut name: Vec<u16> = name_str.encode_utf16().collect();
                        name.push(0);
                        let mut domain: Vec<u16> = domain_str.encode_utf16().collect();
                        domain.push(0);
                        
                        return Ok((name, domain));
                    } else {
                        // Fallback if parsing fails
                        let mut name: Vec<u16> = resolved.encode_utf16().collect();
                        name.push(0);
                        let mut domain: Vec<u16> = "Unknown".encode_utf16().collect();
                        domain.push(0);
                        return Ok((name, domain));
                    }
                }
